        ...
        <motion.div
          ...
          className={`pointer-events-auto flex items-center gap-3 rounded-xl border p-4 shadow-2xl backdrop-blur-md min-w-[280px] max-w-sm ${
            toast.type === 'error'
              ...
              : 'border-[#5865F2]/40 bg-brand-surface/95 text-brand-text shadow-xl'
          }`}
        >
          {toast.type === 'error' ? (
            <AlertTriangle className="h-5 w-5 shrink-0 text-rose-400" />
          ) : (
            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#5865F2]/20 text-[#5865F2]">
              <CheckCircle2 className="h-4 w-4" />
            </div>
          )}

          <div className="flex-1 text-xs">
            <p className="font-semibold text-brand-text">{toast.title || 'Discord Broadcast'}</p>
            <p className="text-brand-muted text-[11px] mt-0.5">{toast.message}</p>
          </div>

          <button
            onClick={onDismiss}
            className="rounded p-1 text-brand-muted hover:text-brand-text transition-colors cursor-pointer"
          >
            <X className="h-4 w-4" />
          </button>
        </motion.div>
