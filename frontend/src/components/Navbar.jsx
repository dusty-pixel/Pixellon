        ...
        <div className="flex w-full items-center justify-between px-4 sm:px-8 lg:px-12 py-3.5">
          ...
          <div className="hidden md:flex items-center gap-2.5">
            ...
            <button
              ...
              className="relative inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-mono font-medium transition-all cursor-pointer border border-surface-700 bg-brand-surface text-brand-text hover:border-brand-accent/50 hover:shadow-xs"
            >
              {isDark ? (
                <>
            ...
            <button
              ...
              className={`relative inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-mono font-medium transition-all cursor-pointer border ${
                hasDiscord
                  ? 'border-[#5865F2]/50 bg-[#5865F2]/15 text-[#8a94fd] hover:bg-[#5865F2]/25'
            ...
            {isAuthenticated && user ? (
              <Link
                to="/profile"
                className="btn-surface inline-flex items-center gap-2 px-3 py-1.5 text-xs font-mono"
              >
                <img
                  src={user.avatar || 'https://images.unsplash.com/photo-1566492031773-4f4e44671857?w=100&auto=format&fit=crop&q=80'}
                  alt={user.username}
                  className="h-5 w-5 rounded-full object-cover border border-brand-accent/40"
                />
                <span className="font-semibold text-brand-text max-w-[90px] truncate">{user.username}</span>
              </Link>
            ) : (
              <Link
                to="/signin"
            ...
            <div className="pt-2 border-t border-surface-700 space-y-2">
              ...
              {isAuthenticated && user ? (
                <NavLink
                  to="/profile"
                  onClick={() => setMobileOpen(false)}
                  className="w-full flex items-center justify-between rounded-lg bg-surface-800 border border-surface-700 px-3 py-2 text-xs font-mono text-brand-text"
                >
                  ...
                  <span className="text-[10px] text-brand-accent uppercase font-mono">Profile</span>
                </NavLink>
              ) : (
                <NavLink
                  to="/signin"
                  onClick={() => setMobileOpen(false)}
                  className="btn-primary w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium"
                >
